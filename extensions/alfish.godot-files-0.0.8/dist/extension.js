...
async function sha512(s) {
    ...
    return Array.prototype.map.call(new Uint8Array(await crypto.subtle.digest('SHA-512', new TextEncoder().encode(s))), (b) => b.toString(16).padStart(2, '0')).join('');
}
...
async function godotVersionOfProject(projectDirUri) {
    let t;
    try {
        t = new TextDecoder().decode(await vscode_1.workspace.fs.readFile(vscode_1.Uri.joinPath(projectDirUri, 'project.godot')));
    }
    catch {
        return null;
    }
    ...
    if (m && m[1]) {
        m = m[1].split(/\s*,\s*/g).map(s => s.match(/^"((\d+)\.(\d+)[^"\\]*)"$/))
            .find(m => m);
        if (m && m[1])
            return { api: m[1], major: +m[2], minor: +m[3] };
    }
...
async function resPathPreview(resPath, document, token) {
    ...
    try {
        ...
        try {
            ...
            if (token.isCancellationRequested)
                return null;
            await vscode_1.workspace.fs.writeFile(imgSrc, new TextEncoder().encode(imgData));
        }
        catch (err) {
            console.error(err);
            return md.appendMarkdown(`[Font file](${resUriStr})`);
...
async function apiDocs(document, className, memberName, token) {
    ...
    if (viewer != 'godot-tools' && await isOnline()) {
        ...
        if (token?.isCancellationRequested)
            return null;
        try {
            return new vscode_1.Location(apiDocsPageUri(className, memberName, gdVersion, apiLocale, viewer), pos0);
        }
        catch (e) {
            console.error(e);
        }
...
function docsPageUri(viewer, urlPath, title, fragment) {
    return vscode_1.Uri.parse(`untitled:${ctx.extension.id}/godot-docs.${viewer}.ide:/${urlPath}/${title}${fragment}`);
}
...
async function loadDocsInTab(urlPath, urlFragment, webviewPanel, token) {
...
section.wy-nav-content-wrap, div.wy-nav-content { margin: auto }
    ...
    const template = docsWebviewInjectHtmlTemplate ?? (docsWebviewInjectHtmlTemplate = new TextDecoder().decode(await vscode_1.workspace.fs.readFile(vscode_1.Uri.joinPath(ctx.extensionUri, 'lang.godot-docs/godot-docs-webview.inject.htm'))));
    const insertVar = { docsUrl, injectHead, urlFragment, classLower, locale, csp };
...
async function onDocsTabMessage(msg) {
    if (msg.navigateTo != undefined) {
        ...
        this._godotFiles_fragment = msg.newFragment;
    }
    else
...
function getActiveDocsUri() {
    ...
    if (!tabInput || tabInput.viewType != GodotDocumentationProvider.viewType || !tabInput.uri) {
        vscode_1.window.showErrorMessage('Could not find an URI of an active Godot Docs Page tab!');
        throw new Error();
    }
    else
        return tabInput.uri;
...
async function activeDocsReload() {
    ...
    const newFragment = webviewPanel._godotFiles_fragment;
